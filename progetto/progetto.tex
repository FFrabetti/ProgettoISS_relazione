...
Un utente è autorizzato se è dotato di un username e una password con cui accedere alla console. Sembra chiaro che sia necessario offrire un servizio di login che, supportato da un meccanismo per la persistenza dei dati, possa permettere di ottenere credenziali per accedere al sistema.\\
...
Fin'ora per permettere l'interazione fra utente e robot abbiamo utilizzato il flag \codescript{-httpserver} offerto dal linguaggio \qa che, se settato per un certo contesto, crea un web server HTTP alla porta 8080 dal quale è possibile inviare i comandi desiderati al robot.

Tale scelta è stata fatta però in una fase prototipale per questioni di rapidità.\\
...
Per farlo abbiamo utilizzato la tecnologia \emph{Node.js} perché permette di scrivere in \emph{Javascript} applicazioni web veloci e scalabili lato server: in particolare il suo framework \emph{Express} fornisce dei template per la generazione di pagine web, utility per il routing, ecc.\\
...
Per quanto riguarda l'interazione tra il server e il resto del sistema, il frontend emette eventi \codescript{usercmd : usercmd(robotgui(CMD))} alla pressione dei tasti di movimento della console ed eventi \codescript{alarm : usercmd(CMD))} per l'avvio e l'interruzione della pulizia. Questi eventi derivano dal fatto che sono gli stessi emessi dalla GUI ottenuta col flag \codescript{-httpserver}.

Per la propagazione degli eventi nel sistema è possibile utilizzare un server Mqtt tramite il quale comunicare messaggi con il modello publish-subscribe. In questo caso tutti gli attori interessati a ricevere o pubblicare informazioni devono essere provvisti del flag \codescript{-pubsub} e dell'istruzione \codescript{pubSubServer} che specifica l'indirizzo e la porta del server Mqtt.

Un'alternativa è quella di aprire una connessione TCP con il contesto principale dell'applicazione ed inviare su di essa gli eventi nel formato di basso livello utilizzato dall'infrastruttura \qa.

Per quanto siano attuabili entrambe le soluzioni, la seconda permette di passare da un frontend all'altro senza bisogno di alcuna modifica. Il passaggio da una all'altra è comunque facilmente realizzabile settando il parametro \codescript{externalActuatorMqtt} presente all'interno del frontend.\\
...
\lstinputlisting[style=style_qa, firstline=24, firstnumber=24]{../it.unibo.raspRobot/src/realrobot.qa}
...
\subsection{Requirement analysis testing}
...
\lstinputlisting[language=Java, firstline=18, lastline=100, firstnumber=18]{../it.unibo.finalTask2018/test/it/unibo/test/crs/SwAgentStartStopEndTest.java}
...
\lstinputlisting[language=Java, firstline=14, lastline=50, firstnumber=14]{../it.unibo.finalTask2018/test/it/unibo/test/crs/SwAgentCleaningTest.java}
...
\lstinputlisting[language=Java, firstline=16, lastline=152, firstnumber=16]{../it.unibo.finalTask2018/test/it/unibo/test/crs/SwAgentObstacleAvTest.java}

\subsection{Implementation testing}
\subsubsection{Hue Lamp interaction test}

\subsubsection{Frontend Server test}

\subsubsection{Virtual Robot test}

\subsubsection{Real Robot test}
%===========================================================================
