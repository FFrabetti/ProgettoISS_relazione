...
L'ambiente è inoltre a sua volta costituito da:
\begin{itemize}[label={--}]
\item 2 sensori (\code{sonar1} e \code{sonar2})
\item ostacoli fissi (pareti) e mobili
\end{itemize}
...
Si noti come la scelta di utilizzare l'evento \texttt{blinkCmd} consente di poter "blinkare" più dispositivi non noti a priori dalla sorgente che emette tali eventi in base alla logica applicativa.

\subsection{Sensors}
Poiché abbiamo a che fare con sensori remoti, ci viene naturale modellarli come attori che rendono disponibili delle informazioni come eventi espressi in un certo formato.

Nello specifico, sono possibili due approcci: a polling, ovvero è chi necessita dell'informazione a farsi carico di fare richiesta periodicamente al sensore, oppure può essere il sensore stesso a pubblicare all'esterno le informazioni quando queste sono disponibili. Scegliamo di adottare questa seconda strategia, poiché meno costosa in termini di messaggi scambiati e poiché garantisce un minor accoppiamento tra le entità coinvolte.

Anche in questo caso il sensore, non conoscendo i componenti interessati, emette semplicemente un evento che può essere percepito da chiunque sia in ascolto.
