...
Dai requisiti si delineano 4 entità:
\begin{itemize}
\item il robot (\texttt{ddr})
...
L'ambiente è a sua volta costituito da:
\begin{itemize}[label={--}]
\item 2 sensori (\code{sonar1} e \code{sonar2})
\item ostacoli fissi (pareti) e mobili
\end{itemize}
...
\caption{Architettura informale}\labelfig{informalRA}
\end{figure}
...
L'\texttt{Hue Lamp} è invece un'entità attiva che ci viene fornita dal committente insieme ad un componente software per interagire con essa...\\

\todo{spiegare come è possibile interagire con l'hue lamp}\\
