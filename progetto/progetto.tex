...
Al momento è presente una \emph{web GUI} fornita automaticamente dall'infrastruttura \qa grazie alla presenza del flag \codescript{-htthserver} presente nel contesto dell'applicazione principale. Questa è in grado di emettere eventi \codescript{usercmd : usercmd(CMD)} alla pressione dei vari pulsanti da parte dell'utilizzatore umano.
...
Poiché non vogliamo che questi comandi vadano persi, introduciamo un componente event-driven (\emph{EventHandler}) che effettui un mapping tra gli eventi usercmd e dei messaggi swagmsg diretti all'agent.

\subsubsection{R-Start}
Alla pressione del pulsante di start, il robot deve trovarsi vicino a \code{sonar1} per poter partire con la pulizia.
